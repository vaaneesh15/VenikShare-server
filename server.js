...
// Секретная фраза для получения админки (лучше хранить в переменной окружения)
const ADMIN_SECRET = 'blesterModGive3319_boshshsh';

// Инициализация таблиц (с новыми полями и таблицами)
async function initDB() {
  try {
    // Таблица пользователей с полем invisible и last_seen
    await pool.query(`
    ...
    await pool.query(`
      CREATE TABLE IF NOT EXISTS messages (
        ...
        sender VARCHAR(50) NOT NULL, -- денормализовано
        text TEXT NOT NULL,
...
async function updateLastSeen(username) {
  // Обновляем только если пользователь не в режиме невидимки
...
app.get('/api/admin/list', async (req, res) => {
  ...
  try {
    const requesterIsAdmin = await isAdmin(requester);
...
// Секретный эндпоинт для получения админки (по фразе)
...
app.post('/api/mails/mark-read', async (req, res) => {
  const { username, ids } = req.body; // ids - массив id писем
...
// ---------- API для личных комнат (остаются без изменений) ----------
...
io.on('connection', (socket) => {
  ...
  socket.on('joinRoom', async ({ roomId, username }) => {
    ...
    try {
      ...
      // Обновляем last_seen (если не невидимка)
      await updateLastSeen(username);
